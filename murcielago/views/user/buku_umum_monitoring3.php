                   ...
                   <table class="table table-bordered table-striped datatable">
                    ...
                    foreach($posts as $l){
                       ...
                       <tr>
                         ...
                         $cari_next_kode = $this->m_data->where('buku_umum',array('id_join' => $l->id_buku_umum,'kode_transaksi' => $cari_kode->kd_transaksi))->row();
                         if($cari_next_kode){
                           $deb = $cari_next_kode->debit;
                          ?>
                          <td>Rp. <a href="javascript:void(0)" data-toggle="modal" data-target="#myModalxa<?php echo $no ?>"><?= number_format($cari_next_kode->debit,2) ?></a></td>
                        <?php }else{
                          $deb = 0;
                          ?>
                        ...
                        <td>Rp. <?= number_format($l->debit,2) ?></td>
                         <td>Rp. <? $salq =  $l->debit - $deb; echo number_format($salq,2)  ?></td>
                         <td>Rp. <?
                          if($no==1){
                             $sal= $salq;
                          }else{
                          ...
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              <div class="form-group">
                                ...
                                if($cari_next_kode){
                                 ?>
                                <form action="<?php echo base_url('buku_umum/update_pikt') ?>" method="post">
                                  <?php
                                  $cari_kode1 = $this->m_data->where('kode_transaksi',array('kode' => 'PPIKT'))->row();
                                  $cari_next_kode1 = $this->m_data->where('buku_umum',array('id_join' => $l->id_buku_umum,'kode_transaksi' => $cari_kode1->kd_transaksi))->row();
                                   ?>
                                  <input type="hidden" name="book_um" value="<?php echo $cari_next_kode->id_buku_umum ?>">
                                  <input type="hidden" name="book_um1" value="<?php echo $cari_next_kode1->id_buku_umum ?>">
                                  <div class="form-group">
                                  ...
                                  <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="nominal_pendapatan" value="<?php echo $cari_next_kode->debit ?>">
                                  </div>
                                  <div class="form-group">
                              ...
                              <?php }else{ ?>
                                <form action="<?php echo base_url('buku_umum/add_pikt') ?>" method="post">
                  ...
                  <!-- </div> -->
                     ...
                     <table class="table table-bordered">
                       <tr style="background-color:#2c3e50;color:#fff;">
                         ...
                         <td> <b>Pengeluaran (Debet)</b> : Rp.&nbsp;<?= number_format($debitqq->total,2) ?></td>
                         <td> <b>Saldo</b> : Rp.&nbsp; <?= number_format($kreditq->total - $debitqq->total,2) ?></td>
                       </tr>
                     </table>
                 </div>
           </div>
