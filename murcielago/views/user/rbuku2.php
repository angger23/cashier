...
<div class="col-md-9" style="padding:0px;">
  ...
  ?>
  <p style="margin: 0px;font-weight: 600;background-color: #9b59b6;color:#fff;padding: 8px;">Saldo : Rp. <?= number_format($kredit); ?></p>
</div>
<div class="col-md-3" style="padding-right:0px;">
  <?
    if($this->uri->segment(3) == 'ac'){
    ?>
    <a href="<?= base_url('buku_anggota/cetak_buku1/'.$this->input->post('start_tgl').'/'.$this->input->post('end_tgl').'/'.$this->input->post('operator').'/PBBKA/kurang') ?>" target="_blank" class="btn btn-info btn-flat pull-right" type="button"><i class="fa fa-print"></i> Print</a>
    <? }else{ ?>
    <a href="<?= base_url('buku_anggota/cetak_buku1/0/0/0/PBBKA/kurang') ?>" target="_blank" class="btn btn-info btn-flat pull-right" type="button"><i class="fa fa-print"></i> Print</a>
    <? } ?>
    <?
    if($this->uri->segment(3) == 'ac'){
    ?>
    <a href="<?= base_url('buku_anggota/export_buku1/'.$this->input->post('start_tgl').'/'.$this->input->post('end_tgl').'/'.$this->input->post('operator').'/PBBKA/kurang') ?>" class="btn btn-success btn-flat pull-right"><i class="fa fa-file-excel-o"></i> Export</a>
    <? }else{ ?>
    <a href="<?= base_url('buku_anggota/export_buku1/0/0/0/PBBKA/kurang') ?>" class="btn btn-success btn-flat pull-right"><i class="fa fa-file-excel-o"></i> Export</a>
    <? } ?>

  </div>
  <br>
 <br>
  <table class="table table-bordered datatable-biasa2">
    <thead>
      <tr>
        <th>No</th>
        <th>Nama Bank</th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <button type="button" class="btn btn-info btn-flat btn-block" data-toggle="modal" data-target="#myModall<?= $l->id_buku ?>2"><i class="fa fa-edit"></i></button>
      ...
      <div id="myModall<?= $l->id_buku ?>2" class="modal fade" role="dialog">
